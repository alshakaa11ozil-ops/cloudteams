...
export default function RecycleBin() {
    ...
    const restoreFileMut = useMutation({
        mutationFn: (fid: number) => restoreFile(teamId, fid),
        onSuccess: () => {
            toast.success('File restored from recycle bin')
            invalidate()
        },
    })
    const restoreFolderMut = useMutation({
        mutationFn: (fid: number) => restoreFolder(teamId, fid),
        onSuccess: () => {
            toast.success('Folder restored from recycle bin')
            invalidate()
        },
    })
    const hardDeleteFileMut = useMutation({
        mutationFn: (fid: number) => hardDeleteFile(teamId, fid),
        onSuccess: () => {
            toast.success('File permanently deleted')
            invalidate()
        },
    })
    const hardDeleteFolderMut = useMutation({
        mutationFn: (fid: number) => hardDeleteFolder(teamId, fid),
        onSuccess: () => {
    ...
    const restoreDocumentMut = useMutation({
        mutationFn: (did: number) => restoreDocument(teamId, did),
        onSuccess: () => {
            toast.success('Document restored from recycle bin')
            invalidate()
        },
    })
    const hardDeleteDocumentMut = useMutation({
        mutationFn: (did: number) => hardDeleteDocument(teamId, did),
        onSuccess: () => {
    ...
    const emptyBinMut = useMutation({
        ...
        onSuccess: () => {
            // WHY toast not alert: alert() blocks the JS thread and looks like a browser warning.
            // toast.success() is non-blocking, styled, and auto-dismisses.
            toast.success('Recycle bin emptied — all items permanently deleted')
            invalidate()
        },
    })
    ...
    const isEmpty = files.length === 0 && folders.length === 0 && documents.length === 0
                            ...
                            <tbody className="divide-y divide-slate-100">
                                ...
                                {documents.map(doc => (
                                    <tr key={`doc-${doc.id}`} className="hover:bg-slate-50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-6 h-6 border border-indigo-100 bg-indigo-50 text-indigo-400 rounded flex items-center justify-center shrink-0">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="font-semibold text-slate-800 truncate max-w-[200px] sm:max-w-xs">{doc.title}</span>
                                                    <span className="text-[11px] font-medium text-slate-400 mt-0.5">CloudTeams Document</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-slate-500 font-medium">DOC</td>
                                        <td className="px-6 py-4 text-slate-500 hidden md:table-cell">
                                            {doc.deletedAt ? format(new Date(doc.deletedAt), 'MMM d, yyyy h:mm a') : 'Unknown'}
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-2 opacity-20 group-hover:opacity-100 transition-opacity">
                                                <button
                                                    onClick={() => restoreDocumentMut.mutate(doc.id)}
                                                    disabled={restoreDocumentMut.isPending}
                                                    className="px-3 py-1.5 bg-blue-50 text-blue-600 rounded text-xs font-bold hover:bg-blue-100 disabled:opacity-50 transition-colors"
                                                >
                                                    Restore
                                                </button>
                                                <button
                                                    onClick={() => {
                                                        if (window.confirm(`Permanently delete document "${doc.title}"?`)) hardDeleteDocumentMut.mutate(doc.id)
                                                    }}
                                                    disabled={hardDeleteDocumentMut.isPending}
                                                    className="px-3 py-1.5 bg-red-50 text-red-600 rounded text-xs font-bold hover:bg-red-100 disabled:opacity-50 transition-colors"
                                                >
                                                    Delete Forever
                                                </button>
