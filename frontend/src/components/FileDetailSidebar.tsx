...
type Tab = 'preview' | 'comments' | 'versions' | 'lock'
...
export default function FileDetailSidebar({
  file, teamId, currentUserId, userRole, onClose,
  ...
  const handleForceUnlock = async () => {
    ...
    try {
      ...
      alert(err?.response?.data?.error ?? 'Force unlock failed.')
    } finally {
      setIsForcing(false)
    }
  }
  ...
  return (
    <div className="w-96 border-l bg-white flex flex-col h-full shadow-xl flex-shrink-0">
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          {!lockToken && !isLocked && (
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-semibold bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors"
            >
      ...
      <div className="flex border-b text-sm px-2 flex-shrink-0">
        {(['preview', 'comments', 'versions', 'lock'] as Tab[]).map(tab => (
          <button
            key={tab}
...
function CommentCard({
  ...
  const editMutation = useMutation({
    ...
    onSuccess: () => { setIsEditing(false); onMutated() },
  })
