...
function FileCard({ file, teamId, currentUserId, folderId, onDelete, onRename, onMove, onSelect, formatBytes, hideDelete }: FileCardProps) {
  ...
  const downloadMutation = useMutation({
    mutationFn: () => downloadFile(file.id, teamId, file.original_name),
  })
    ...
    >
      ...
      {showActions && (
        ...
        ">
          ...
          {/* Edit (Day 2: Collaborative Editor) */}
          {/* Only shown for text, markdown, and Word documents */}
          {(() => {
            const ext = file.original_name.split('.').pop()?.toLowerCase()
            const isEditable = ['txt', 'md', 'docx'].includes(ext || '')
